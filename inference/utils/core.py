...
def imresize(
    ...
    return x
def resize_cv2img(img,scale):
    img = img.astype(np.float)/255.0 
    img = np.transpose(img,(2,0,1))
    tensor = torch.from_numpy(img).unsqueeze(0) 
    resize_numpy = imresize(tensor,scale)[0].numpy()
    resize_numpy = np.transpose(resize_numpy*255.0,(1,2,0))
    return np.clip(resize_numpy,0,255).astype(np.uint8)
